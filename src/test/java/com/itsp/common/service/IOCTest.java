...
public class IOCTest extends ItspJUnit4ClassRunner {
	...
	public void APPLICATION_CONTEXT_IS_RUNNING() {
		...
		Assert.assertNotNull("上下文不能为空", applicationContext.getApplicationContext());
	}

	@Test
	public void BEAN_IS_INIT() {
		Assert.assertNotNull("bean 后置事件为成功初始化", applicationContext.getApplicationContext().getBean(SessionFactionAutowiring.class));
	}
}